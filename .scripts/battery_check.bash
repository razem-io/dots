...
show_popup() {
    WISH_SCRIPT="wm state . withdrawn; tk_messageBox -icon warning -title \"Battery Warning\" 
-message \"${1}\"; exit"
...
show_notify(){
    ...
    notify-send -u critical ${NOTIFY_OPT} "Akku leer" "Bitte Netzteil anschließen."
}
...
main (){
    ...
    while true; do
        ...
        echo "sleeping ${SLEEP_TIME}; current limit ${LIMIT}%; ${POPUP_CLICKED:+Popup was 
clicked}"
        sleep "${SLEEP_TIME}"
    done
} >&2
...
(
    ...
    flock -xn 200 || { show_message "$(basename ${0}): cannot acquire lock ${LOCK_FILE}"; exit 
3; }
